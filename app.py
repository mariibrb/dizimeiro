...
def main():
    ...
    # --- DESIGN RIHANNA (FUNDO RADIAL E NEON) ---
    st.markdown("""
        <style>
        ...
        * {
        ...
        /* Título Centralizado Rosa Vibrante */
        h1 {
        ...
        /* Inputs Branco Puro e Arredondados */
    ...
    with col_a:
        st.markdown('<div class="rihanna-card">📜 CONFIGURE O CNPJ E REGIME NA SIDEBAR PARA AUDITAR AS NOTAS DE TERCEIROS.</div>', unsafe_allow_html=True)
    with col_b:
        st.markdown('<div class="rihanna-card">📁 SUBA SEUS XMLS OU ZIPS. O SISTEMA IDENTIFICA O GRUPO E FILTRA FILIAIS.</div>', unsafe_allow_html=True)

    with st.sidebar:
        st.markdown("<h2 style='color:#FF69B4; text-align:center;'>MENU</h2>", unsafe_allow_html=True)
        cnpj_input = st.text_input("CNPJ DESTINATÁRIO")
        meu_regime = st.selectbox("REGIME", ["Simples Nacional", "Regime Normal"])
        minha_uf = st.selectbox("UF", list(ALIQUOTAS_INTERNAS.keys()), index=25)
        cnpj_alvo = limpar_cnpj(cnpj_input)

    up_ger = st.file_uploader("📂 RELATÓRIO GERENCIAL", type=['csv'])
    ...
    if up_files and cnpj_alvo:
        try:
            ...
            if df_final.empty:
                st.warning("NENHUM XML EXTERNO ENCONTRADO.")
                return
            usar_ger = False
            if up_ger:
                try:
            ...
            st.download_button("📥 BAIXAR AUDITORIA", out.getvalue(), "Auditoria_Dizimeiro.xlsx")
