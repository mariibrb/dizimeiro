...
# --- CONFIGURAÇÃO E ESTILO (DESIGN RIHANNA REFINADO) ---
st.set_page_config(page_title="DIZIMEIRO", layout="wide", page_icon="💰")

def aplicar_estilo_rihanna_v7():
    st.markdown("""
        <style>
        ...
        /* Tipografia Montserrat (Removido Negrito Global) */
        * {
            font-family: 'Montserrat', sans-serif !important;
            font-weight: 400; /* Texto normal, sem negrito pesado */
        }
        ...
        /* Uploader: Borda Tracejada Rosa e Fundo Branco Puro */
        ...
        /* Botões Rosa com Efeito Neon Real */
        ...
        div.stButton > button:hover, div.stDownloadButton > button:hover {
            ...
            box-shadow: 0 0 25px rgba(255, 105, 180, 0.8), 0 0 40px rgba(255, 105, 180, 0.4) !important;
            background-color: #FF1493 !important; /* Rosa mais forte no hover */
        }
...
# --- LOGICA FISCAL DIZIMEIRO 6.0 (INTOCADA) ---
ALIQUOTAS_INTERNAS = {
...
# --- INTERFACE ---
aplicar_estilo_rihanna_v7()
st.markdown("<h1>💰 O DIZIMEIRO</h1>", unsafe_allow_html=True)

c1, c2 = st.columns(2)
with c1: st.markdown('<div class="rihanna-card">📜 CONFIGURE O CNPJ NA SIDEBAR PARA AUDITAR AS NOTAS DE TERCEIROS EXTERNOS.</div>', unsafe_allow_html=True)
with c2: st.markdown('<div class="rihanna-card">📁 XMLS OU ZIPS (MATRIOSCA) SÃO ACEITOS. O SISTEMA FILTRA AS FILIAIS.</div>', unsafe_allow_html=True)

with st.sidebar:
    st.markdown("### 🔍 CONFIGURAÇÃO")
    cnpj_in = st.text_input("CNPJ AUDITADO")
    regime = st.selectbox("REGIME", ["Simples Nacional", "Regime Normal"])
    uf_des = st.selectbox("UF", list(ALIQUOTAS_INTERNAS.keys()), index=25)
    cnpj_alvo = limpar_cnpj(cnpj_in)

if cnpj_alvo:
    up_ger = st.file_uploader("1. RELATÓRIO GERENCIAL (CSV/OPCIONAL)", type=['csv'])
    up_xml = st.file_uploader("2. XMLS OU ZIPS", type=['xml', 'zip'], accept_multiple_files=True)

    if up_xml and st.button("🚀 INICIAR APURAÇÃO NEON"):
        all_itens = []
        for f in up_xml:
        ...
        if not df.empty:
            ...
            st.markdown(f"<h2>TOTAL: R$ {df['DIFAL_Recolher'].sum():,.2f}</h2>", unsafe_allow_html=True)
            st.dataframe(df[df['DIFAL_Recolher'] > 0][['Nota', 'Emitente', 'Analise', 'DIFAL_Recolher']])
            
            out = io.BytesIO()
            df.to_excel(out, index=False)
            st.download_button("📥 BAIXAR AUDITORIA NEON", out.getvalue(), "Auditoria_Dizimeiro.xlsx")
else:
